// SudokuAI.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
vector<int> backtrackCSP_MRV(vector<int> queue, vector<int> assignment, int count, vector<int> domain[81], const bool C[81][81]);
...
int main()
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		auto start = high_resolution_clock::now();
		AC3(domain, C);
		vector<int> result = backtrackCSP_MRV(queue, assignment, count, domain, C);
		auto stop = high_resolution_clock::now();
		fillDisplayBoard(table, result);
		displayBoard(table);
		auto duration = duration_cast<microseconds>(stop - start);
		cout << "Time taken by backtrack MRV: "
			<< duration.count() << " microseconds" << endl;
		//avDuration += duration.count();
		//cnt++;
		//system("pause");

	}
	//avDuration = (avDuration / cnt);
	//cout << "Average Time taken by backtrack MRV: "
		//<< avDuration.count() << " microseconds" << endl;
...
bool AC3(vector<int> domain[81], const bool C[81][81])
{
	...
	while ((queue.size() > 0) && (queue.size() < 10000)) {
		...
		if (revise(xi, xj, domain, C)) {
			...
			for (int i = 0; i < 81; i++) {
				if ((C[xi][i] == true) && (i != xj)) {
					queue.push_back(make_pair(i, xi));
				}
			}
		}
...
{
	bool revised = false;
	int DiSize = domain[xi].size();
	int DjSize = domain[xj].size();
	vector<int> Di = domain[xi];
	vector<int> Dj = domain[xj];

	if ((C[xi][xj] == true) && (DjSize == 1)) {
		for (int i = 0; i < DiSize; i++) {
			for (int j = 0; j < DjSize; j++) {
				if (Di[i] == Dj[j]) {
					Di.erase(Di.begin() + i);
					DiSize -= 1;
					domain[xi] = Di;
...
vector<int> backtrackCSP_MRV(vector<int> queue, vector<int> assignment, int count, vector<int> domain[81], const bool C[81][81])
{
	...
	if (queue.size() == 1) {
		if (queue.front() == 999) //if assignment complete
			return assignment;
	}

	if (queue.size() != 0)	//only fills the queue once
		search = false;

	int j = 0, i = 0;
	while (search) {	//fill the MRV queue
		vector<int> D = domain[j];
		if (D.size() == i)
			queue.push_back(j);
	...
	queue.erase(queue.begin());
	if (queue.size() == 0)
		queue.push_back(999);  //flag to show that the queue has finished
	...
	for (int i = 0; i < n; i++)
	{
		...
		{
			assignment[r] = value;
			result = backtrackCSP_MRV(queue, assignment, count + 1, domain, C);
			if (result.size() > 0)
				return result;
			assignment[r] = 0;
		}
